...
void poisson(int n) {
	...
	*/
	...
	double time_start;
	double duration;
	int *work = mk_int_array(2, false);  // index, number of rows
...
// Load balancing is handled on each processor, rather than having the added overhead of send, receives and broadcasts while other processors are waiting
